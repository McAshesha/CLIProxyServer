...
// Helpers for SOCKS5 handshake parsing
static int read_socks5_greeting(int client_fd, uint8_t *n_methods)
{
    uint8_t header[2];
...
static int select_no_auth_method(int client_fd)
{
    uint8_t method_resp[2] = { SOCKS5_VERSION, 0x00 };
...
static int read_request_header(int client_fd, uint8_t *command, uint8_t *address_type)
{
    uint8_t header[4];
...
// Build the address field for the SOCKS5 reply
static size_t append_address_field(uint8_t *response, size_t offset, const struct sockaddr *addr)
{
    if (addr->sa_family == AF_INET)
    {
        const struct sockaddr_in *a4 = (const struct sockaddr_in *)addr;
        response[offset++] = SOCKS5_ATYP_IPV4;
        memcpy(response + offset, &a4->sin_addr.s_addr, 4);
        offset += 4;
        memcpy(response + offset, &a4->sin_port, 2);
        offset += 2;
    }
    else if (addr->sa_family == AF_INET6)
    {
        ...
        memcpy(response + offset, &a6->sin6_addr, 16);
        offset += 16;
        memcpy(response + offset, &a6->sin6_port, 2);
        offset += 2;
    }
    else
    {
        response[offset++] = SOCKS5_ATYP_IPV4;
        offset += 6; // zeroed by initializer
    }
               ...
               const struct sockaddr *bind_address, socklen_t bind_address_len)
{
    ...
    offset = append_address_field(response, offset, bind_address);
...
// Destination parsing for different address types
static int parse_dest_ipv4(int client_fd, struct sockaddr_storage *dest, socklen_t *addr_len)
{
    uint8_t buf[6];
...
static int parse_dest_ipv6(int client_fd, struct sockaddr_storage *dest, socklen_t *addr_len)
{
    uint8_t buf[18];
...
static int parse_dest_domain(int client_fd, struct sockaddr_storage *dest,
                             socklen_t *addr_len, uint8_t command)
{
    uint8_t dom_len;
...
static int parse_destination(int client_fd, uint8_t address_type,
                             ...
                             socklen_t *addr_len, uint8_t command)
{
    switch (address_type)
    {
        case SOCKS5_ATYP_IPV4:
            return parse_dest_ipv4(client_fd, dest, addr_len);
        case SOCKS5_ATYP_IPV6:
            return parse_dest_ipv6(client_fd, dest, addr_len);
        case SOCKS5_ATYP_DOMAIN:
            return parse_dest_domain(client_fd, dest, addr_len, command);
        default:
                     ...
                     struct sockaddr_storage *out_destination, socklen_t *out_dest_len)
{
    ...
    // 1) Greeting phase
    if (read_socks5_greeting(client_fd, &n_methods) < 0)
    {
        return -1;
    }

    // 2) Method selection (NO AUTH)
    if (select_no_auth_method(client_fd) < 0)
    {
        return -1;
    }

    // 3) Client request header
    if (read_request_header(client_fd, &command, &addr_type) < 0)
    {
        return -1;
    }
    *out_command = command;

    // 4) Read and parse destination
    if (parse_destination(client_fd, addr_type, &dest, &addr_len, command) < 0)
    {
        return -1;
    }

    // 5) Send CONNECT reply if needed
