...
int server_start()
{
    ...
    while (true)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            void *current_ud    = events[i].data.ptr;
            int   current_fd    = *(int *)current_ud;
            int   current_events = events[i].events;
            ...
            if (current_events & EPOLLIN)
            {
                ...
                {
                    tunnel_read_handle(current_fd, current_ud);
                }
            }
            else if (current_events & EPOLLOUT)
            {
                tunnel_write_handle(current_fd, current_ud);
            }
            else
            {
                LOG_ERROR("Unexpected epoll events: 0x%x on fd=%d",
                          current_events, current_fd);
            }
        }
    }
...
int server_init(char *host, char *port, char *username, char *passwd)
{
    ...
    {
        LOG_ERROR("Failed init_server, getaddrinfo failed error=%s",
                  gai_strerror(errno));
        return -1;
    }

    addrinfo_t *ai_ptr = address_list;
    int         listenfd = -1;
    for (; ai_ptr != NULL; ai_ptr = ai_ptr->ai_next)
    {
        listenfd = socket(ai_ptr->ai_family,
                          ai_ptr->ai_socktype,
                          ai_ptr->ai_protocol);
        if (listenfd < 0)
        {
    ...
    if (listenfd < 0)
    {
        LOG_ERROR("Failed init_server, listenfd create failed errno=%s",
                  strerror(errno));
    ...
    setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR,
               (void *)&reuse_flag, sizeof(reuse_flag));

    if (bind(listenfd, ai_ptr->ai_addr, ai_ptr->ai_addrlen) != 0)
    {
    ...
    LOG_INFO("Listening socket fd=%d bound to %s:%s",
             listenfd, host, port);
